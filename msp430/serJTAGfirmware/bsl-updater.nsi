...
!define IMAGE_DIR       "."                     ;location of the image file
!define IMAGE           "firmware.a43"          ;name of the image file
!define BSLOPTIONS      ''                      ;options to firmware_updater.exe
!define MSPGCCBIN       ".\bin"                 ;location of firmware_updater.exe
OutFile                 "pyserJTAG-firmware-updater.exe"           ;The file to write the installer to
...
InstallDir "$TEMP\firmware_updater"     ;The default installation directory
...
do_download:
    ...
    ClearErrors
    nsExec::ExecToLog '"$INSTDIR\firmware_updater" ${BSLOPTIONS} -c ${PORT} "$INSTDIR\${IMAGE}"'
    Pop $0
    IntCmp $0 0 ext_ok
;ext_err:
