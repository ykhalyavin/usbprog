...
void eeprom_write(char * buf){
	...
	if(usbprog.cmdpackage == 1){
		...
		for(;bufindex < bytes; bufindex++){
			eeprom_write_byte_isp((char)(pgmmode.address >> 8), (char)pgmmode.address, buf[bufindex]);
			pgmmode.address++;
		}
	}
	else {
		// alle anderen packete, no packet header, data only
		if(pgmmode.numbytes > 64) {
			bytes = 64;
			pgmmode.numbytes = pgmmode.numbytes - 64;
        	usbprog.longpackage = 1;
		...
		for(bufindex = 0; bufindex < bytes; bufindex++){
			eeprom_write_byte_isp((char)(pgmmode.address >> 8), (char)pgmmode.address, buf[bufindex]);
			pgmmode.address++;
		}
	}
...
void USBFlash(char *buf)
{
	...
	if(usbprog.longpackage) {
		...
		case CMD_LOAD_ADDRESS:
			// set given address
			// was pgmmode.address = (buf[1] << 24) | (buf[2] << 16) | (buf[3] << 8) | buf[4]; // compiler warning "wrong type width"
